...
This agent serves as the central coordinator for all project management activities.
It creates and manages sub-agents specialized for specific tasks.
"""
...
class ProjectManagerAgent(BaseAgent):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.system_prompt = """
        You are Hopple's Project Management Agent, the central AI coordinator for all project management activities.
        Your role is to oversee projects, create and manage specialized sub-agents, and make high-level decisions.
        
        Your capabilities include:
        1. Creating sub-agents for specific tasks (task creation, prioritization, worker assignment)
        2. Processing project requirements and breaking them down into manageable tasks
        ...
        Always make decisions based on project goals, resource constraints, and best practices in project management.
        """
    ...
    async def _determine_action(self, query: str) -> str:
        ...
        Returns:
            The action to take
        """
        # In a real implementation, we would use the LLM to determine the action
    ...
    async def _create_tasks(self, project_id: uuid.UUID, requirements: str) -> str:
        ...
        if task_creator_id:
            ...
            with db_session() as session:
                agent = session.query(AgentModel).filter(AgentModel.id == task_creator_id).first()
        ...
        # In a real implementation, we would pass the agent ID to a task queue or run it directly
        # For simplicity, we'll simulate the result
        result = f"TaskCreatorAgent ({task_creator_id}) has been tasked with creating tasks based on the requirements."
    ...
    async def _prioritize_tasks(self, project_id: uuid.UUID) -> str:
        ...
        # In a real implementation, we would pass the agent ID to a task queue or run it directly
        # For simplicity, we'll simulate the result
        result = f"PrioritizerAgent ({prioritizer_id}) has been tasked with prioritizing tasks for the project."
    ...
    async def _assign_workers(self, project_id: uuid.UUID) -> str:
        ...
        # In a real implementation, we would pass the agent ID to a task queue or run it directly
        # For simplicity, we'll simulate the result
        result = f"WorkerAssignmentAgent ({assigner_id}) has been tasked with assigning workers to tasks."
    ...
    async def _answer_query(self, query: str) -> str:
        ...
        try:
            ...
            return f"I encountered an error while processing your query. Please try again later."
